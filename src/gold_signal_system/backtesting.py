...
class BacktestingEngine:
    ...
    def run(self, candles: list[dict], source_timeframe: str = "1m") -> dict[str, Any]:
        ...
        for i in range(self.config.min_window, len(candles) - self.config.horizon_candles):
            ...
            pnl = outcome["realized_rr"] - self.config.commission_per_trade
            equity += pnl
    ...
    def _simulate_outcome(self, rec: FinalRecommendation, future_candles: list[dict]) -> dict[str, float | str]:
        ...
        entry = rec.entry_price
        sl = rec.stop_loss
        tp1 = rec.take_profit_1 or entry
        tp2 = rec.take_profit_2 or entry
        tp3 = rec.take_profit_3 or entry
        ...
        for row in future_candles:
            ...
            if direction == SignalDirection.BUY:
                ...
                if high >= tp3:
                    outcome = "TP3_HIT"
                    realized_rr = abs(tp3 - entry) / risk
                    break
                if high >= tp2:
                    outcome = "TP2_HIT"
                    realized_rr = abs(tp2 - entry) / risk
                    break
                if high >= tp1:
                    outcome = "TP1_HIT"
                    realized_rr = abs(tp1 - entry) / risk
            else:
                ...
                if low <= tp3:
                    outcome = "TP3_HIT"
                    realized_rr = abs(entry - tp3) / risk
                    break
                if low <= tp2:
                    outcome = "TP2_HIT"
                    realized_rr = abs(entry - tp2) / risk
                    break
                if low <= tp1:
                    outcome = "TP1_HIT"
                    realized_rr = abs(entry - tp1) / risk
